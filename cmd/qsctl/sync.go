...
var SyncCommand = &cobra.Command{
	...
	Short: i18n.Sprint("sync between local directory and QS-Directory"),
	Long: i18n.Sprint(`qsctl sync between local directory and QS-Directory. The first path argument
...
key(file) will be overwritten only if the source one newer than destination one.`),
	Example: utils.AlignPrintWithColon(
		i18n.Sprint("Sync local directory to QS-Directory: qsctl sync . qs://bucket-name"),
		i18n.Sprint("Sync QS-Directory to local directory: qsctl sync qs://bucket-name/test/ test_local/"),
		i18n.Sprint("Sync skip updating files that already exist on receiver: qsctl sync . qs://bucket-name --ignore-existing"),
	),
...
func initSyncFlag() {
	SyncCommand.Flags().BoolVar(&syncInput.IgnoreExisting, "ignore-existing", false,
		i18n.Sprint(`skip creating new files in dest dirs, only copy newer by time`))
}
