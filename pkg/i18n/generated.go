...
NOTICE: qsctl will not tee the content to stdout like linux tee command does.
`)
	_ = message.SetString(tag, `skip sync files already exist in dest dirs, only copy those not synced`, `skip sync files already exist in dest dirs, only copy those not synced`)
